...
export function resolveRoute(pathname) {
  ...
  if (projectMatch) return { id: "project", params: { projectId: projectMatch[1] } };

  const chapterMatch = path.match(/^\/open-thread\/(santa-ana|san-juan-capistrano)$/);
  if (chapterMatch) return { id: "openThreadChapter", params: { chapter: chapterMatch[1] } };

  if (path === "/partner-a-drop") return { id: "partners", params: {} };
  if (path === "/wholesale") return { id: "wholesale", params: {} };
  if (path === "/operations") return { id: "operations", params: {} };

  const route = publicRoutes.find((entry) => entry.path === path);
