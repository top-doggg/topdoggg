...
def build_back():
    # Top anchored: begins at Printify safe top, mapped higher in mockup shoulder area.
    art = visible(Image.open(SOURCE_ART).convert('RGBA'))
    art = fit(art, 3260, 4075)
    canvas = Image.new('RGBA', CANVAS, (0, 0, 0, 0))
    canvas.alpha_composite(art, ((CANVAS[0] - art.width) // 2, SAFE[1]))
    return canvas


def build_front():
    # Smaller pocket logo. Simple original-style halo, no animated stars.
    lettering = fit(visible(Image.open(LETTERING).convert('RGBA')), 500, 205)
    mark = Image.new('RGBA', (lettering.width + 90, lettering.height + 78), (0, 0, 0, 0))
    d = ImageDraw.Draw(mark)
    cx = mark.width // 2
    d.ellipse((cx - 92, 13, cx + 92, 54), outline=BLACK, width=9)
    d.ellipse((cx - 75, 22, cx + 75, 46), outline=BLACK, width=4)
    mark.alpha_composite(lettering, (45, 66))
    mark = visible(mark)
    mark = fit(mark, 455, 305)

    canvas = Image.new('RGBA', CANVAS, (0, 0, 0, 0))
    cx_print, cy_print = 2780, 1095
...
def make_mock(front, back, destination):
    ...
    # Higher on the shirt preview so characters start at shoulder area.
...
def write_placement(path):
    path.write_text(
        ...
        '- Front placement: smaller wearer\'s left chest Better Together script with simple original-style halo\n'
...
def main():
    ...
    source_front = SOURCE_DIR / 'young-boyz-small-original-halo-pocket-print.png'
    source_back = SOURCE_DIR / 'young-boyz-shoulder-top-back-print.png'
    ...
    refresh_master_zip()
    print('corrected pocket scale/halo and raised back mockup placement')
